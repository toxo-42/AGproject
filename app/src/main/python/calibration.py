...
profile.py(classify_profile)는 강/보통/약 3단계 중 하나로 "분류"하는데,
normal↔strong 경계가 실측 데이터에서 자꾸 겹쳐서 불안정했다
(§진행상황_및_로드맵.md Phase B). 그 대안으로 이 모듈은 분류 없이
"이 사람의 캘리브레이션 데이터로 직접 임계값을 계산"한다 — 버킷 경계 자체가
없으니 경계에서 오분류될 일도 없다.
